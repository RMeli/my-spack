...
class Pika(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant("sanitize", default=False, description="Add sanitizer")
    variant(
        "stdexec",
        default=False,
    ...
    def cmake_args(self):
        ...
        args += [
            ...
            self.define_from_variant("PIKA_WITH_STACKOVERFLOW_DETECTION", "sanitize"),
        ]

        if spec.satisfies("+sanitize"):
            args.append("-DCMAKE_CXX_FLAGS='-fsanitize=leak -fsanitize=address -fstack-check -fno-omit-frame-pointer'")
